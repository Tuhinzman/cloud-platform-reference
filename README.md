...
| [ADR-0019](docs/decisions/0019-bound-immutable-image-controls-for-aws-managed-runtime-images.md) | Bounds the immutable-image controls where AWS itself delivers the image and exposes no supported control over the reference pulled, measured on the EKS managed add-ons: ten compensating controls, an identity check that is detective rather than preventive, and a declared limitation stated plainly. Supersedes only ADR-0017's pin-and-mirror requirement and the blocking clause enforcing it, for that class alone. |
